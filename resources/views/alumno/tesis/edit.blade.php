<x-alumno-layout>
    @if ($observacion != '' and ($tesis->tesis_status==8 or $tesis->tesis_status==9))
        <div role="alert">
            <div class="bg-red-500 text-white font-bold rounded-t px-4 py-2">
              Observaciones de {{($tesis->tesis_status==8)?' Secretaria':' Direccion'}}:
            </div>
    ...
    <form action="{{route('tramite.tesis.update',['tesi'=>$tesis])}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
        @if ($tesis->tesis_status != 9)
            <div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">                    
                    ...
                    <div class="mt-2">
                        <img id="previewVoucher" src="{{$tesis->vouchers->first()->voucher_url}}" alt="">
                    </div>
                </div>
            </div>
        </div>
