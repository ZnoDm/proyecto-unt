<x-alumno-layout>
    ...
    @endphp
    @if ($observacion != '' and ($tesi->tesis_status==10 or $tesi->tesis_status==11))
    <div role="alert">
        <div class="bg-red-500 text-white font-bold rounded-t px-4 py-2">
        Observaciones
        </div>
        <div class="border border-t-0 border-red-400 rounded-b bg-red-100 px-4 py-3 text-red-700">
        <p>{{$observacion->to_detalle}}</p>
        </div>
    </div>
    <hr class="my-3">
    @endif
    ...
    <form action="{{route('tramite.tesis.informefinal.update',$tesi)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('put')
                    ...
                    <div class="mb-2 grid grid-cols-2 gap-4 px-2">
                        <div class="col-span-2"> 
                            <label for="nro">Número de Operacion:
                                <input type="text" name="nro" id="nro" class="w-full" value="{{old('nro',$tesi->vouchers->last()->voucher_nro)}}">
                                @error('nro')
                                    <span>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mt-2"><img id="previewVoucher" src="{{$tesi->vouchers->last()->voucher_url}}" alt="">
                          </div>
                </div>
            </div>
        </div>
    ...
    <div id="modalFUT" class="modal">
        <div class="modal-content" overflow: scroll;>
            ...
            <embed id="previewFUT" type="application/pdf" style="height: 92%; width: 100%;" src="{{$tesi->futs->last()->fut_url}}">
        </div>
    </div>
    <!-- Modal Informe Final -->
    <div id="modalPlan" class="modal">
        <div class="modal-content" overflow: scroll;>
            ...
            <embed src="{{$tesi->tesis_file_informefinal}}" id="previewPlan" type="application/pdf" style="height: 92%; width: 100%;">
        </div>
    </div>
    @push('scripts')
        <script>
